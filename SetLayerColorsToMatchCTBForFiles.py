...
#                        '/runscript="-RunPythonScript (G:\\0000 CAD LIBRARY\\Scripts\\rhino\\SetLayerColorsToMatchCTB.py) save exit"',
...
for i, selected_file in enumerate(selected_files):
    with open(os.environ['TEMP'] + "\\madness" + str(i) + ".cmd", "w") as madness:
        madness.write('start "" /min "C:\\Program Files\\Rhino 8 WIP\\System\\Rhino.exe" /nosplash /runscript="-RunPythonScript (G:\\0000 CAD LIBRARY\\Scripts\\rhino\\SetLayerColorsToMatchCTB.py) save exit" ' + chr(34) + selected_file + chr(34))
        madness.flush()
